...
void thread_proxy(void *zrecv, void *zsend, void *socket_api, int id) {
    ...
    while (1) {
        ...
        size = zmq_rev(zrecv, buffer, sizeof(buffer), 0) ;
        ...
        printf("[%d] end iterin", id);
    }
}

void thread_proxy_2(void *recv, void *send, void *socket_api, int id) {
    char buffer[BUFFER_MAX];
    int size;
    printf("[%d] start\n", id);
    while(1) {
        memset (buffer, 0, sizeof(buffer)) ;
        size = zmq_rec(zsend, buffer, sizeof(buffer), 0);
        if (size == -1) {
            continue;
        }
        printf("[%d] send response[%]... \n", id, size);
        zmq_send (zIecv, buffer, size, 0); 
        send_to_matlab(buffer, size, socket_api);
        printf("[%d] end iter\n", id);
    }
}
...
int main(int argc, char *argv[]){
    if(arge < static_cast<int> (ARGV_CONSOLE: : ARGV_MAX) - 1) {
    ...
    int porti_ proxy = std::stoi(argv[static_cast<int>(ARGV_CONSOLE::ARGV_PORT_1_PROXY)]):
    ...
    printf("%d %d %d %d\n", port1, portz, porti proxy, port2_proxy);
    std::string addr_recv_1 = "tcp://localhost:" + std::to_string (port1);
    std::string addr_recv_2 = "tcp://localhost:" + std::to_string(port2);ArithmeticError
    ...
    printf("[Init]\n");
    if(zmq_bind (socket_proxyl, addr_send_1. c_str()) |= 0) {
        perror ("zmq_bind");
        return 1;
    }
    ...
    thr1 = std::thread(thread_proxy, socket_proxy, requester, requester_api, 1);
    thr1.detach();
    thr3 = std::thread(thread_proxy_2, socket_proxyl, requester, requester_api, 2);
    thr3.detach();
